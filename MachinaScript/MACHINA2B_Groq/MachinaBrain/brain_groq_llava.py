...
# Sends the encoded image to the local AI model for generating a thought / environment analysis.
# This example requires you to have access a local multimodal LLM running.
# We recommend LMstudio running a small model like LLaVA or Obsidian.
# Make sure the model size will fit your vRAM (video card ram). 
# We suggest picking a very small model around 3-4gb for very fast inference.
# More instructions at the github repo user manual.
def get_image_thought(base64_image, base_url="http://localhost:1234/v1"):
    print("Thinking about the image, wait a second.")
    client = OpenAI(base_url=base_url, api_key="not-needed")
    completion = client.chat.completions.create(
        model="local-model", #placeholder text - this don't need to be filled.
        messages=[
            {
                "role": "system",
                "content": "Describe the objects positions in the image very shortly.",
            },
            {
                "role": "user",
                "content": [
...
def generate_machina_script(thought):
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            ...
            {
                "role": "user",
                "content": "input: " + thought + " note: output machinascript code only. if you have to say anything else, do it on the *say* skill.",
            }
        ],
